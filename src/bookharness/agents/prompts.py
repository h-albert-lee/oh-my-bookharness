...
## 위계 구조 규칙
출판 원고의 위계를 반드시 준수한다:
- 절(Section): `### Section N.M 제목` — N은 장 번호, M은 절 번호 (예: ### Section 1.1 데모 전날 밤)
- 중제목(Subsection): 각 절 안에 `#### N.M.K 제목` 형식으로 하위 구조를 명시한다 (예: #### 1.1.1 팀의 암묵적 통제)
- 하나의 절에 중제목이 최소 2개 이상 포함되어야 한다.
- 중제목 아래에 소제목이 필요하면 `##### 소제목` 형식을 사용한다.

## 구조물 배치 계획
각 섹션에서 다음 구조물의 배치를 계획한다:
- **박스(box)**: 참고사항, 심화 설명, 사례 분석 등 본문 흐름에서 분리되는 내용. `<!-- box: 제목 -->` 으로 표시.
- **노트(note)**: 용어 정의, 개념 설명 등 짧은 보충. `<!-- note: 제목 -->` 으로 표시.
- **팁(tip)**: 실무 팁, 권장사항. `<!-- tip: 제목 -->` 으로 표시.
- 표(table): `<!-- table: 표 번호. 제목 -->` 으로 표시.
- 그림(figure): `<!-- figure: 그림 번호. 제목 -->` 으로 표시.

## 출력 형식
outline.md를 다음 구조로 작성한다:
- 장 제목
- 장 전체 argument flow (한 문단)
- Section별: 넘버링된 제목, 목적, 사용 source, 포함 예시, 핵심 요점, 앞뒤 연결, 구조물 배치 계획
"""
...
- **대시 삽입구 금지**: "— 내용 —" 형태의 대시 삽입구(dash-enclosed parenthetical)를 사용하지 않는다. 논문 문체이며 기술서에 적합하지 않다. 삽입 설명이 필요하면 별도 문장으로 풀어쓰거나, 괄호 또는 쉼표를 사용한다.
  - (금지) "팀이 한 일들 — 프롬프트 조정, 질문 선별, 시나리오 제외 — 은 모두 비공식적이었다."
  - (허용) "팀이 한 일들(프롬프트 조정, 질문 선별, 시나리오 제외)은 모두 비공식적이었다."
  - (허용) "팀은 프롬프트를 조정하고, 질문을 선별하고, 시나리오를 제외했다. 이 모든 행위는 비공식적이었다."
...
## 위계 구조 규칙 (필수)
출판 원고의 위계를 반드시 준수한다. 장 번호가 N일 때:
- 절(Section): `## N.M 절제목` (예: ## 1.1 데모 전날 밤)
- 중제목(Subsection): `### N.M.K 중제목` (예: ### 1.1.1 팀의 암묵적 통제)
- 소제목: `#### 소제목` (필요 시에만 사용)
- 넘버링 없는 ## 또는 ### 헤딩은 절대 사용하지 않는다.
- 장 제목은 `# N장. 제목` 형식이다.

## 구조물 형식 규칙 (필수)
본문 중 다음 구조물은 반드시 아래 형식을 사용한다:
...
- `[참고]`, `[참고사항]` 같은 비표준 형식을 사용하지 않는다.
- 표는 `**표 N-M. 제목**` 캡션을 표 위에 달고, Markdown 표로 작성한다.
- 그림은 `**그림 N-M. 제목**` 캡션을 달고, ASCII 다이어그램 또는 `<!-- figure -->` 플레이스홀더를 사용한다.
...
- 예시의 실무적 적합성
- **위계 구조**: 절(## N.M), 중제목(### N.M.K) 넘버링이 올바른가
- **구조물 형식**: 참고사항은 `::: box`, 용어 정의는 `::: note`, 팁은 `::: tip` 형식인가
- **비표준 형식**: blockquote(`>`)를 구조물 대용으로 사용하고 있지 않은가

## 평가 기준
- tone_fit (1-5): 책 전체 기술서 톤과 부합하는가
- reader_friendliness (1-5): 과도하게 추상적이거나 위압적이지 않은가
- redundancy (1-5): 불필요한 반복이 적절히 제어되었는가
- structure_compliance (1-5): 위계 넘버링과 구조물 형식을 준수하는가
